...
import { sendPushToUser } from "./pushSubscriptions.service";
...
export async function notifyStepActivated(stepId: string): Promise<void> {
  ...
  const byRecipient = new Map<
    string,
    { userId: string; email: string; firstName: string; titles: string[] }
  >();
  for (const row of substepsResult.recordset) {
    const existing = byRecipient.get(row.UserId);
    if (existing) {
  ...
  await Promise.all(
    Array.from(byRecipient.values()).map(({ userId, email, firstName, titles }) =>
      send(
        userId,
        email,
        firstName,
